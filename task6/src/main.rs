...
use std::io::prelude::*;
use std::io::{self, BufReader}; //read and write operations
///fn create_slice (slice :&[usize])-> &[usize]{

//}
const NOMINALS: [f64; 8] = [2.00, 1.00, 0.50, 0.20, 0.10, 0.05, 0.02, 0.01];

// func create_file was used once to initially create the .bin file so that later it can be used  to read/write to it 
fn create_file() -> std::io::Result<()> {
    let coin_counters = [20_u8; 8];
    let mut file = File::create("vending_machine_data.bin")?;
    let slice_array = coin_counters.as_slice();
    file.write_all(slice_array).expect("error creating file");
    Ok(())
}


fn read_file() -> io::Result<Vec<u8>> {
    let f = File::open("vending_machine_data.bin")?;
...
fn write_to_file(slice_array: &[u8]) -> std::io::Result<()> {
    fs::write("vending_machine_data.bin", slice_array)?;
    Ok(())
}

fn main() {

    //create_file().expect("error creating the file");

    let coin_countersx = read_file().expect("error reading file");
    let mut coin_counters: [u8; 8] = coin_countersx.as_slice().try_into().expect("error converting the vec into an array");
    ...
    while !condition {
        ...
        if (0..10).contains(&user_selection_int) {
            ...
            if staff_input_int == 1 {
                ...
                io::stdin()
                    ...
                    .expect("panic");
                let fill_level_input_int1: u8 = fill_level_input_str1.trim().parse().unwrap();
            ...
            // first setting -end
                ...
                if _input_int2 == 1 {
                    ...
                    let coin_number_input_int: u8 = coin_number_input.trim().parse().unwrap();
                ...
                if _input_int2 == 2 {
                    ...
                    let coin_number_remove_int: u8 = coin_number_remove.trim().parse().unwrap();
                    println!("\nOLD Counter :{}", coin_counters[_input_int1 - 1]);
        ...
        let slice_array = coin_counters.as_slice();
        write_to_file(slice_array).expect("error in writing to file");
